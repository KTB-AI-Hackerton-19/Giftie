...
- Qwen이 카테고리, 추천 이유, 검색어, 감사 메시지 생성
- Tavily가 실제 상품을 검색
- 의미 관련성, 개별 상품 URL, 가격을 검증
...
| 영역 | 역할 | 현재 상태 |
|---|---|---|
| FE | 모바일 화면, 입력 폼, 목록·캘린더·추천 결과 표시, API 호출 | Next.js 기본 골격과 라이브러리 설정 단계 |
| BE | 인증, 사용자·사람·선물·카테고리·캘린더·알림 저장 및 조회, S3, SSE, AI 호출 | 주요 도메인과 API 구현됨 |
| AI-Service | 입력 정규화, 이미지/직접 입력 오케스트레이션, Qwen 추천, Tavily 검색 | 추천·검색 실제 동작, 일부 작업 mock |

세부 구현과 실행법은 [FE](FE_Readme.md), [BE](BE_Readme.md),
[AI Service](AI_Service_Readme.md) 문서를 참고합니다.
...
| AI | Python, FastAPI, Pydantic, Qwen, MLX(Mac), Transformers(GPU), Tavily, httpx |
...
AI Service의 현재 공개 API는 다음 두 개입니다.
...
현재 BE의 `AiExtractionClient`, `AiRecommendationClient`는 각각 `/extract`,
`/recommendations`를 호출하고 서로 다른 DTO를 사용합니다. AI-Service의 실제 API와
아직 연결되지 않았으므로 AWS 배포 전 다음 중 하나로 통일해야 합니다.

1. 권장: BE 클라이언트를 AI-Service의 두 agent API와 최종 응답 DTO에 맞게 수정
2. 대안: AI-Service에 BE 구형 계약용 어댑터 라우터 추가

공개 업무 API를 두 개로 유지하기 위해 1번을 권장합니다. 상세 매핑은
[BE 통합 문서](BE_Readme.md#ai-service-통합)에서 확인할 수 있습니다.
...
사용자
  ...
  → Spring Boot BE (private subnet)
       ├─ RDS MySQL
       ├─ S3
       └─ FastAPI AI-Service (GPU private subnet)
            ├─ Qwen model
            └─ Tavily HTTPS API
```
...
- Qwen은 GPU당 worker 1개를 기본으로 해 모델 중복 적재를 방지합니다.
...
- 이미지 분석 모델 실제 연결
- Google Calendar MCP 실제 등록
- 푸시 알림 또는 이메일·메신저 발송
- 추천 상품 클릭·선택·구매 이력
- 사용자 피드백 기반 개인화 추천
- 부조금과 경조사 전용 입력·추천 정책
...
| 네 작업 중 부분 실패 | 병렬 실행 및 작업별 `READY`/`ERROR` 반환 |
| BE-AI DTO 불일치 | 통합 DTO 테스트와 OpenAPI 기준 계약 고정 |
| 개인정보·이미지 노출 | private S3, presigned URL, private network, 로그 마스킹 |

## 15. 통합 완료 체크리스트

- [ ] FE 화면을 Giftie 디자인으로 구현
- [ ] FE의 API base URL을 BE 주소로 설정
- [ ] BE의 AI 클라이언트 URL과 DTO를 AI-Service 계약으로 변경
- [ ] 직접 입력 전체 흐름 E2E 테스트
- [ ] S3 이미지 업로드와 AI 이미지 입력 E2E 테스트
- [ ] 캘린더·알림·추천 결과 DB 반영 정책 확정
- [ ] AI-Service mock 세 작업의 담당 구현 연결
